...
# Windows fallback: if the file at the macOS path is not a real Win32 binary, try the
# standard Windows Chrome install path instead. (The macOS path resolves to an E:\Applications
# shim on this machine, which exists but is not executable by Windows Python.)
